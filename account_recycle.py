...
DELETE flow — 100% confirmed HAR se (25 Aug), pure API calls hain, koi
...
hain (generic patterns) — agar fail ho, exact selector Wajid se confirm
karna hoga (DOM inspect karke).
"""
import asyncio
...
SIGNUP_URL = "https://app.heygen.com/signup"  # NOTE: exact URL confirm karna
MAGIC_LINK_WAIT_AFTER_SUBMIT = 3  # seconds — turnstile solve + request jaane ka waqt

# Text-match se click karta hai (CSS-selector guess se zyada robust — HeyGen
# ke buttons mein stable class/id nahi hai, sirf text-content hai). Ye
# same-origin JS injection hai (page.evaluate se) — jaisa poore codebase mein
# (render.py, tts_generate.py) already hota hai, naya tareeka nahi hai.
_CLICK_BY_TEXT_JS = """
(() => {{
    const target = {text!r};
...
# React-controlled input mein seedha .value = X kaam nahi karta (React apna
# state track karta hai) — native setter + input-event dispatch chahiye,
# yehi standard tareeka hai React inputs ko script se bharne ka.
_FILL_EMAIL_JS = """
(() => {{
...
async def trigger_signup(browser, email_addr: str, status_cb=None):
    """
    Signup page kholta hai, 3-step UI flow chalata hai:
      1. "Use email" button click (default screen Google/Apple dikhata hai)
      2. Email input bharo
      3. (kabhi-kabhi) Cloudflare captcha-click — best-effort, agar na aaye
         to skip
      4. "Send a secure magic link" button click

    Page KHUD magic_link request bhejती hai (turnstile_token uske apne JS
    se aata hai — hum use replay nahi karte, isीliye poora UI-flow chalana
    zaroori hai, seedha API-call nahi bhej sakte).
    """
    ...
    # Cloudflare captcha kabhi-kabhi aata hai (Wajid ke mutabiq) — cross-origin
    # iframe hai isliye plain JS se click nahi ho sakta (browser security),
    # zendriver ki real CDP-click chahiye. Best-effort — agar na mile ya fail
    # ho, chup-chap aage badh jaate hain (shayad is baar aaya hi na ho).
    ...
    try:
        ...
        pass  # captcha nahi tha is baar — normal hai
...
async def login_via_magic_link(browser, magic_link_url: str, status_cb=None):
    ...
    IMPORTANT (confirmed 26 Aug via debug-log, real data):
    Login turant (4-6 sec) ho jaata hai — 'heygen_is_login', 'heygen_session',
    'heygen_sid', 'heygen_track_uid' cookies turant aa jaate hain, page
    '/onboarding' pe settle ho jaata hai. 'heygen_token' cookie is (magic-
    link) login-flow mein KABHI NAHI aata — wo galat assumption tha (shayad
    kisi PURANE/ALAG login-method ka cookie hai). Onboarding-page se hume
    koi lena-dena nahi (Wajid confirm kiya — cookie login ke turant baad hi
    ready ho jaati hai, onboarding-click ki zaroorat nahi).

    FIX: 'heygen_is_login' ya 'heygen_session' ki maujoodgi check karte hain
    (ye asli, jaldi-aane-wale signal hain), 'heygen_token' ka wait nahi.
    """
    async def _status(msg):
        if status_cb:
            await status_cb(msg)
    ...
    while elapsed < 40:
        ...
        current_url = getattr(page, "url", None) or "(unknown)"
        print(f"[recycle-debug] {elapsed}s: page-url={current_url}  heygen-cookies-present={heygen_cookie_names}")
